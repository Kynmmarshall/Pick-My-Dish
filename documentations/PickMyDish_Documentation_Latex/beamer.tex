...
\begin{frame}{Testing \& Quality Metrics}
    ...
    \begin{columns}[T]
        ...
        \vspace{0.35cm}
        \begin{tikzpicture}[
            bar/.style={rounded corners=4pt, minimum height=0.6cm, anchor=west},
            label/.style={font=\footnotesize\bfseries, anchor=west}
        ]
            \node[label] at (0,2.1) {Unit};
            \draw[bar, fill=Accent!70, draw=Accent!80, minimum width=3.1cm] (0,1.6) rectangle ++(3.1,0.6);
            \node[font=\scriptsize\bfseries] at (3.3,1.9) {62\%};

            \node[label] at (0,0.8) {Integration};
            \draw[bar, fill=Primary!70, draw=Primary!80, minimum width=2.0cm] (0,0.3) rectangle ++(2.0,0.6);
            \node[font=\scriptsize\bfseries] at (2.2,0.6) {40\%};

            \node[label] at (0,-0.5) {E2E (happy path)};
            \draw[bar, fill=Secondary!60, draw=Secondary!70, minimum width=1.6cm] (0,-1.0) rectangle ++(1.6,0.6);
            \node[font=\scriptsize\bfseries] at (1.8,-0.7) {32\%};

            % Target line
            \draw[dashed, gray] (2.8,-1.2) -- (2.8,2.6);
            \node[font=\tiny\bfseries, align=center, gray] at (2.8,2.5) {75\% target};
        \end{tikzpicture}
        \vspace{0.2cm}
        \textcolor{DarkBg!70}{\scriptsize Next: prioritize E2E and integration to hit 75\%.}
        ...
        \begin{tabularx}{0.98\textwidth}{|l|c|c|}
            ...
            \hline
            API Response (p95) & \textcolor{Accent}{\textbf{<500ms}} & \textbf{410ms} \\
            \hline
            DB Query (p95) & \textcolor{Accent}{\textbf{<300ms}} & \textbf{240ms} \\
            \hline
            Uptime (30d) & \textcolor{Accent}{\textbf{99.8\%}} & \textbf{99.6\%} \\
            \hline
        \end{tabularx}
        \vspace{0.2cm}
        \textcolor{DarkBg!70}{\scriptsize Actions: cache hot recipes, tighten API p95, improve uptime alerts.}
    \end{columns}
\end{frame}
